...
var Markov = new function() {
	...
	return {
		...
		addList: function (race,names) { Name[race] = names; },
		loadList: function (race) { construct(Name[race]); },
		deleteList: function (race) { if (race) { delete Name[race]; }; },
		addName: function (race,name) { Name[race].push(name); },
		getList: (function (race) {return Name[race];}),
		// Get the number of entries for a race
		getLength: (function (race) {
			if (race) {
				return Name[race].length;
			}
			return 0;
		}),
		// Get the list of names for a race in CSV format
		getCSV: (function (race) {
			var csv = "";
			if (race) {
				if (Name[race]) {
					for (name in Name[race]) {
						if (csv !== "") {
							csv += ", ";
						}
						csv += Name[race][name];
		...
		setCSV: (function (race, inputString) {
			var csv = [];
			if (race && inputString) {
